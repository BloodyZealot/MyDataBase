...
class Solution {
	...
	struct dictnode
	{
		string v;
		bool used;
		bool  operator<(const dictnode& n)
		{
			return v < n.v;
		}
	};
...
public:
	vector<vector<string> > findLadders(string start, string end, unordered_set<string> &dict)
	{
		...
		bool ended = false;
		//initialize
		vector<dictnode> vdict;
		vdict.reserve(dict.size());
		unordered_set<string>::iterator a = dict.begin(), b = dict.end();
		for (; a != b; ++a)
			vdict.push_back(dictnode{ *a,false });
		sort(vdict.begin(), vdict.end());
		vector<vector<snode> > front(1, vector<snode>(1, snode{ -1, start, -1 })),
			back(1, vector<snode>(1, snode{ -1, end, -1 }));
		//search
		vector<vector<snode> >::reverse_iterator fptr, bptr;
		vector<dictnode>::iterator begp, endp;
		vector<snode>::iterator fbeg, fend, bbeg, bend;
		int parent;
		endp = vdict.end();
		int size;
		int compresult;
		while (1)
		{
			fbeg = front.rbegin()->begin();
			fend = front.rbegin()->end();
			bend = back.rbegin()->end();
			for (; fbeg != fend; ++fbeg)
			{
				bbeg = back.rbegin()->begin();
				for (; bbeg != bend; ++bbeg)
				{
					if (compare(fbeg->v, bbeg->v)!=-1)
					{
						ended = true;
						oneslo.clear();
						oneslo.push_back(fbeg->v);
						parent = fbeg->parent;
						fptr = ++front.rbegin();
						while (parent != -1)
						{
							oneslo.push_front((*fptr)[parent].v);
							parent = (*fptr++)[parent].parent;
						}
						oneslo.push_back(bbeg->v);
						parent = bbeg->parent;
						bptr = ++back.rbegin();
						while (parent != -1)
						{
							oneslo.push_back((*bptr)[parent].v);
							parent = (*bptr++)[parent].parent;
						}
						result.push_back(vector<string>(oneslo.begin(), oneslo.end()));
					}
				}
			}
			if (ended)
				break;
			size = front.rbegin()->size();
			front.push_back(vector<snode>());
			fptr = ++(front.rbegin());
			for (int i = 0; i != size; ++i)
			{
				begp = vdict.begin();
				...
				{
					if (!begp->used)
					if ((compresult = compare(begp->v, (*fptr)[i].v)) != -1
						&& compresult != (*fptr)[i].changepos)
					{
						begp->used = true;
						front.rbegin()->push_back(snode{ i, begp->v, compresult });
					}
				}
			}
			fbeg = front.rbegin()->begin();
			...
			size = back.rbegin()->size();
			back.push_back(vector<snode>());
			bptr = ++back.rbegin();
			for (int i = 0; i != size; ++i)
			{
				begp = vdict.begin();
				...
				{
					if (!begp->used)
					if ((compresult = compare(begp->v, (*bptr)[i].v)) != -1
						&&compresult != (*bptr)[i].changepos)
					{
						begp->used = true;
							back.rbegin()->push_back(snode{ i, begp->v, compresult });
					}
				}
			}
			if (back.rbegin()->size() == 0)
				back.pop_back();
		}
		cout << clock() - timer;
		return result;
...
int main()
{
	...
	Solution s;
	for each (vector<string> v in s.findLadders(start, end, dict))
	{
		cout << '[';
